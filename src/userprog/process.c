...
void push_arguments (const char *[], int cnt, void **esp);
...
/* Starts a new thread running a user program loaded from
  ...
  //  add by student
tid_t
process_execute (const char *cmdline) //add by student
{
  ...
  cmdline_copy = palloc_get_page (0); //palloc_get_page (0) = 4KB
  if (cmdline_copy == NULL) {
    goto execute_failed;
  }
  strlcpy (cmdline_copy, cmdline, PGSIZE); //copy cmdline to cmdline_copy
  ...
  strlcpy (file_name, cmdline, PGSIZE); //copy cmdline to file_name
  ...
  pcb->pid = PID_INITIALIZING; //pid is process_id of process
  ...
  sema_init(&pcb->sema_initialization, 0); //synchronization??
  ...
  execute_failed:
  // release release allocated memory and return
...
/* Waits for thread TID to die and returns its exit status.  If
   it was terminated by the kernel(nếu TID kết thúc bởi kernel thì trả về -1) 
   (i.e. killed due to an
